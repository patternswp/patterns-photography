...
/* Local */
class PatternsPhotographySettingsApi {
    constructor() {
        this.endpoint = 'patterns-photography/v1/settings';
    ...
    bindEvents() {
        document.addEventListener('click', (event) => {
            if (event.target.classList.contains('patterns-photography-gsn-close')) {
                this.modifySettings('hide_get_started_notice', true);
            }
            else if (event.target.classList.contains('patterns-photography-rn-permanent-close')) {
                this.modifySettings('remove_review_notice_permanently', true);
            }
            else if (event.target.classList.contains('patterns-photography-rn-temporary-close')) {
                this.modifySettings('remove_review_notice_temporary_date_time', 1);
            }
            else if (event.target.classList.contains('patterns-photography-install-plugins')) {
    ...
    updateUI() {
        if (this.settings.hide_get_started_notice) {
            const noticeElement = document.getElementById('patterns-photography-gsn');
            if (noticeElement) {
        ...
        if (!this.isLoadReview()) {
            const reviewNoticeElement = document.getElementById('patterns-photography-review-notice');
    ...
    async installPlugins() {
        const plugins = PatternsPhotographyLocalize.recommended_plugins;
        ...
        window.location.replace(PatternsPhotographyLocalize.theme_info_url);
    }
}

document.addEventListener('DOMContentLoaded', () => {
    new PatternsPhotographySettingsApi();
});
