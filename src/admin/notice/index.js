/*CSS*/
/* global PatternsPhotographyLocalize */
...
class PatternsPhotographySettingsApi {
	...
	bindEvents() {
		document.addEventListener( 'click', ( event ) => {
			...
			if ( target.closest( '.patterns-photography-gsn-close' ) ) {
				this.modifySettings( 'hide_get_started_notice', true );
			} else if (
				target.closest( '.patterns-photography-rn-permanent-close' )
			) {
				this.modifySettings( 'remove_review_notice_permanently', true );
			} else if (
				target.closest( '.patterns-photography-rn-temporary-close' )
			) {
				this.modifySettings(
				...
				const installBtn = target.closest(
					'.patterns-photography-install-plugins'
				);
				if ( installBtn ) {
					installBtn.classList.add(
						'patterns-photography-processing'
					);
					this.installPlugins();
				}
			}
	...
	updateUI() {
		...
		if ( ! this.isLoadReview() ) {
			const reviewNoticeElement = document.getElementById(
				'patterns-photography-review-notice'
			);
